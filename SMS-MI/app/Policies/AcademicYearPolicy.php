<?php

namespace App\Policies;

use App\Models\User;
use App\Models\AcademicYear;
use Illuminate\Auth\Access\HandlesAuthorization;

class AcademicYearPolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the academicYear can view any models.
     *
     * @param  App\Models\User  $user
     * @return mixed
     */
    public function viewAny(User $user)
    {
        return true;
    }

    /**
     * Determine whether the academicYear can view the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\AcademicYear  $model
     * @return mixed
     */
    public function view(User $user, AcademicYear $model)
    {
        return true;
    }

    /**
     * Determine whether the academicYear can create models.
     *
     * @param  App\Models\User  $user
     * @return mixed
     */
    public function create(User $user)
    {
        return true;
    }

    /**
     * Determine whether the academicYear can update the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\AcademicYear  $model
     * @return mixed
     */
    public function update(User $user, AcademicYear $model)
    {
        return true;
    }

    /**
     * Determine whether the academicYear can delete the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\AcademicYear  $model
     * @return mixed
     */
    public function delete(User $user, AcademicYear $model)
    {
        return true;
    }

    /**
     * Determine whether the user can delete multiple instances of the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\AcademicYear  $model
     * @return mixed
     */
    public function deleteAny(User $user)
    {
        return true;
    }

    /**
     * Determine whether the academicYear can restore the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\AcademicYear  $model
     * @return mixed
     */
    public function restore(User $user, AcademicYear $model)
    {
        return false;
    }

    /**
     * Determine whether the academicYear can permanently delete the model.
     *
     * @param  App\Models\User  $user
     * @param  App\Models\AcademicYear  $model
     * @return mixed
     */
    public function forceDelete(User $user, AcademicYear $model)
    {
        return false;
    }
}
